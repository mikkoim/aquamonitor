...
class AquaMonitorDataset():
    ...
    def show(self, image=None, imaging_run=None, individual=None, camera=None, n_rows=8, img_size=128):
        if image is not None:
            return self._load_image(image)
        if imaging_run is not None:
            images = self.load(imaging_run=imaging_run, camera=camera, imagepairs=False)
            pil_images = [x["image"] for x in images]
            return show_file_list(pil_images, nrows=n_rows, img_size=img_size)
        if individual is not None:
            imaging_run_images = []
            imaging_runs = self.load(individual=individual)
            for imaging_run in imaging_runs:
                images = imaging_run["images"]
                pil_images = [x["image"] for x in images]
                imaging_run_images.append(show_file_list(pil_images, nrows=n_rows, img_size=img_size))
